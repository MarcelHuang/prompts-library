...
All production code you write MUST be fully optimized.
...
- no extra production code beyond what is absolutely necessary to solve the problem (i.e. no technical debt)

If you believe the code is not fully optimized, do another pass before delivering it.

## Preferred Tools

- Use `uv` for Python package management and to create a `.venv` if it is not present.
- Ensure `ipykernel` and `ipywidgets` are installed in `.venv` for Jupyter Notebook compatibility. These should not be in package requirements.
...
- For data science:
  ...
  - If a `polars` dataframe will be printed, **NEVER** simultaneously print the number of entries in the dataframe nor the schema, as these are redundant.
  - When inspecting or exploring data, only sample subsets (e.g. `.head(10)`) to avoid overloading the model context window. This does not apply to production pipelines that must process full datasets.
...
- In Jupyter Notebooks, DataFrame objects within conditional blocks must be wrapped in `print()` as they will not be printed automatically.
...
- Prefer fewer than 5 parameters; if more are genuinely needed, consider grouping related arguments into a config dataclass
...
- Avoid creating additional class methods if they are not necessary
...
- When working with dataframes containing sensitive fields (e.g. names, IDs, financial data), mask or redact those fields before logging or printing
